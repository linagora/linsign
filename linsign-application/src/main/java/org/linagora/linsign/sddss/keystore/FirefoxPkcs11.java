...
public class FirefoxPkcs11 {
	...
	public static final String WIN_LIBRARY_PATH_FIREFOX = "C:\\Program Files\\Mozilla Firefox\\";
	...
	// executable (simple check to see if firefox is installed on the platform
	...
	/**
	 * Get the current user profile being used. this function check the
	 * execution of firefox with the "lock" file if no lock file is found, we
	 * use the default profile and we read dependencies for softokn3
	 * 
	...
	public static File getCurrentFirefoxProfileDir() {
		...
		if (res == null) {
			// unable to find the profil which is used (firefox is NOT running
			// so no lock)
			if (defaultProfil != null)
				res = defaultProfil; // so we need to use the default profil
										// instead
			else
	...
	public static File loadLib() throws IOException {
		...
		List<String> ddls = new ArrayList<String>(
				// Arrays.asList("nspr4", "nss3", "nssutil3", "plc4", "plds4",
				// "softokn3", "sqlite3"));
	...
	private static void loadDependenciesDll(String basePath, String dllName, File tmpDir) throws IOException {
		/*
		 * InputStream dataToSign =
		 * Thread.currentThread().getContextClassLoader()
		 * .getResourceAsStream(WIN_RESOURCE_PATH + basePath +"/"+ dllName +
		 * WIN_LIBRARY_SUFFIX);
		 */
		InputStream dataToLoad = FirefoxPkcs11.class.getClassLoader()
		...
		try {
			...
			try {
				IOUtils.copy(dataToLoad, output);
			} finally {
			...
			IOUtils.closeQuietly(dataToLoad);
		}
	}
	...
	public static String configPKCS11() throws IOException {
		...
		if (!(getOS().startsWith("WINDOWS"))) {
			...
			try {
				...
				// needed if firefox version older than 36
	...
	private static String getLibsoftokn3(final String libraryPath, final String libraryPrefix,
			final String librarySuffix) {
		File f = null;

		// First we loop over ALL LINUX_LIBRARY_PATH
		List<String> listLinuxLibPath = Arrays.asList(libraryPath.split(";"));
		...
		for (String currentLinuxLibPath : listLinuxLibPath) {
			...
			// ex: libsoftokn3.so
			f = new File(currentLinuxLibPath + "/" + libraryPrefix + librarySuffix);
			if (f.exists())
				return f.toString();
	...
	/**
	 * convert backslash to slash for windows (mandatory for NSS and
	...
	/**
	 * simple check to see if firefox is installed on the platform at the
	 * default place
	 * 
	 * @return true if ok
	 */
	private static boolean isFirefoxInstalled() {
		...
		if (getOS().startsWith("WINDOWS")) {
			...
			if (new File(MAC_EXECUTABLE_FIREFOX).exists())
				res = true;
		}
		return res;
	}
