...
import org.gsafeproject.signature.client.LinSign;
...
public class KeyStoreUtils {
	...
	private static List<KeyStore> getMSKeyStoreWithSunMSCAPI() throws KeystoreAccessException {
		...
		String OS = System.getProperty("os.name").toUpperCase().trim();
		String JVM = System.getProperty("java.runtime.version").toUpperCase().trim();
	...
	public static KeyStore getPKCS11Store(File PKCS11LibFile, char[] password, String tokenName, String configp11)
		...
		try {
			...
			Provider pkcs11prov = new sun.security.pkcs11.SunPKCS11(configStream);
	...
	public static void releasePKCS11Store(String tokenName) {
		...
		sun.security.pkcs11.SunPKCS11 old = (sun.security.pkcs11.SunPKCS11) Security
				.getProvider("SunPKCS11-" + tokenName);
	...
	/**
	 * This method is a workaround for scenarios when multiple entries have the
	 * same alias. Since the alias is the only "official" way of retrieving an
	 * entry, only the first entry with a given alias is accessible. See:
	 * https://joinup.ec.europa.eu/software/sd-dss/issue/problem-possible-
	 * keystore-aliases-collision-when-using-mscapi
	 *
	 * @param keyStore
	...
	public static List<KeyStore> getFirefoxPKCS11KeyStore() throws KeystoreAccessException, IOException {
		...
		System.out.println(configPKCS11);
			...
			try {
				...
				if (e.getCause() instanceof FailedLoginException) {
					...
					if (e.getMessage().equals("load failed")) {
						...
						JOptionPane.showMessageDialog(null, "Le certificat est verrouillé");
					} else {
						JOptionPane.showMessageDialog(null, "Il semblerait qu'aucun mot de passe a été indiqué");
					}
				}else
					e.printStackTrace();
			} catch (Exception e) {
				if (!e.getMessage().equals("PKCS11 not found"))
					JOptionPane.showMessageDialog(null,
							"Impossible de charger le certificat, la raison : " + e.getMessage());
	...
	public static KeyStore loadKeystoreWithPassword(KeyStore ks) {
		try {
			...
			int action = JOptionPane.showConfirmDialog(null, pwd, "Entrer le mot de passe principal de firefox",
					JOptionPane.OK_CANCEL_OPTION);

			if (action < 0) {
				JOptionPane.showMessageDialog(null, "Annuler, X ou Echap bouton selectionné");
				return null;
			} else {
			...
			JOptionPane.showMessageDialog(null, "Erreur de chargement du keystore principal");
			return null;
		}
	}
}
