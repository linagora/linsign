...
public class KeyUsageSignatureFilter extends KeystoreEntryFilter {
	...
	boolean checkNonRepudiation = false;
	
	
	public KeyUsageSignatureFilter(boolean checkNonRepudiation){
		this.checkNonRepudiation = checkNonRepudiation;
	}
	
	
	@Override
	public boolean accept(KeyStoreEntry cert) {
		
		boolean res;
		
		if(cert.getKeyusage()==null) // no keyusage ?
			res = false;
		else {
			
			if(!checkNonRepudiation) {
				if(cert.getKeyusage()[0]) //digital signature
